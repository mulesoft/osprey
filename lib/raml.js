const dtexp = require('datatype-expansion')
...
function expandTypes (raml) {
  const ctx = _createTypesContext(raml.types)
  if (!ctx) {
    return raml
  }
  ...
  function _expand (data, ctx) {
    ...
    const isExpandable = data.type &&
      data.type.length === 1 &&
      NON_EXPANDABLE_TYPES.indexOf(data.type[0]) === -1

    if (isExpandable) {
      const expanded = dtexp.expandedForm(data, ctx)
