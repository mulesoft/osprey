    ...
    for (var i = 0; i < securedBy.length; i++) {
      ...
      if (typeof secured === 'string') {
        if (authenticate.hasOwnProperty(secured)) {
          middleware = composeMiddleware(authenticate[secured](null, path))
      ...
      Object.keys(secured).forEach(function (key) {
        if (authenticate.hasOwnProperty(key)) {
          middleware = composeMiddleware(authenticate[key](secured[key], path))
  ...
  return function (req, res, done) {
    ...
    function next (err) {
      ...
      if (index === handlers.length) {
        ...
        // Use the errors status code by default.
        var status = errors[0].status

        // Override the status code when multiple errors exist.
        if (errors.length > 1) {
          status = 401
        }
...
/**
 * Compile middleware into a single function.
 *
 * @param  {Function|Array} handlers
 * @return {Function}
 */
function composeMiddleware (handlers) {
  // Functions are already perfect.
  if (typeof handlers === 'function') {
    return handlers
  }

  if (!Array.isArray(handlers)) {
    throw new TypeError('Handlers must be a function or array')
  }
