...
function createSecurity (model, opts) {
  ...
  const securitySchemes = model.declares
    .filter(dec => dec instanceof wp.model.domain.SecurityScheme)
  ...
  securitySchemes.forEach(scheme => {
    ...
    const res = handler(scheme, handlerOptions, name)
  ...
  middleware.push(resources(model.encodes.endPoints, (method, path) => {
    ...
    securedBy.forEach(secured => {
      ...
      // Security scheme applied without parameters
      if (!secured.settings) {
        const name = secured.name.value()
        if (Object.prototype.hasOwnProperty.call(authenticate, name)) {
          middleware = compose(authenticate[name](null, path))
      ...
      // TODO: Stopped here
      // Security scheme applied WITH parameters
      Object.keys(secured).forEach(key => {
        if (Object.prototype.hasOwnProperty.call(authenticate, key)) {
          ...
          debug('Path "%s" is not secured with "%s"', path, key)
        }
      })
    })
