...
function createOAuth2Handler (scheme, opts, key) {
  ...
  var accessTokenUri = new urllib.Url(options.accessTokenUri || settings.accessTokenUri).path
  ...
  if (
    ...
    var authorizationUri = new urllib.Url(options.authorizationUri || settings.authorizationUri).path
