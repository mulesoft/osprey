...
function createOAuth2Handler (scheme, opts, key) {
  ...
  // It's only possible to define a single OAuth2 flow in RAML 1.0 so pick
  // the first one.
  const flow = (scheme.settings.flows || [])[0]
  const scopes = flow.scopes
    ? flow.scopes.map(s => s.name.value())
    : []
  const authorizationGrants = scheme.settings.authorizationGrants
    ? scheme.settings.authorizationGrants.map(s => s.value())
    : []
  ...
  invariant(
    validPathEnding(flow.accessTokenUri.option, accessTokenUri),
  ...
  if (
    ...
    const authorizationUriObj = new urllib.URL(
      ...
      flow.authorizationUri.option)
    const authorizationUri = authorizationUriObj.pathname + authorizationUriObj.search

    invariant(
      validPathEnding(flow.authorizationUri.option, authorizationUri),
   ...
   *
   * @param  {webapi-parser.OAuth2Settings} schemeSettings
   * @return {Function}
   */
  function handler (schemeSettings) {
    if (!schemeSettings) {
      return authenticate
    }
    ...
    // It's only possible to define a single OAuth2 flow in RAML 1.0 so pick
    // the first one.
    const flow = (schemeSettings.flows || [])[0]
