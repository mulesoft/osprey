...
function createOAuth2Handler (scheme, opts, key) {
  ...
  const scopes = arrify(scheme.settings.scopes)
  ...
  const accessTokenUriObj = new urllib.URL(options.accessTokenUri || settings.accessTokenUri)
  ...
  if (
    ...
    const authorizationUriObj = new urllib.URL(options.authorizationUri || settings.authorizationUri)
    ...
    const immediate = immediateAuthorization ? function (client, user, scope, done) {
